...
class Test(Base):
    ...
    def spiro(self, x, y, color, poly_radius):
        ...
        if len(self.x_list) >= 1:
            x_near = abs(x - self.x_list[0]) < 0.01
            y_near = abs(y - self.y_list[0]) < 0.01
    ...
    def update(self):
        d = self.R - self.r
        Rate = 0.015
        xc = d * np.cos(2*self.wR)
        yc = d * np.sin(self.wR)**2 
        xs = xc + (self.r/6)* (np.cos(2*self.wr + self.x_offset))
        ys = yc + self.r/3 * np.sin(self.wr + self.y_offset)

        
        if self.iter == 0 and self.capture_iter == 0:
            self.config = f'{self.R}_megan_{round(self.x_offset, 2)}_{round(self.y_offset, 2)}'
        ...
        if not self.draw_dots or (self.iter > 2000):
            ...
            self.x_offset += 0.1
            self.capture_iter += 1
            self.r = 3 + 2.5*sin(self.x_offset/2)
            self.draw_dots = True
