...
class Test(Base):
        ...
        for j in np.arange(0, Z / 2, np.sqrt(3) * RADIUS): # DRAW IN VERTICAL COLUMNS
            ...
            for i in np.arange(-Z / 2, Z / 2, 3 / 2 * RADIUS):
                if counter % 2 == 0:
                    ...
                    R = np.sqrt(x0 ** 2 + y0 ** 2)
                    phi0 = np.arccos(x0 / R)
                    x1 = R * np.cos(phi0 + phi)
                    y1 = R * np.sin(phi0 + phi)
                    p1 = PolygonGeometry(
                        sides = 6,
                        radius = x0*RADIUS,
                        x = x1,
        ...
        for j in np.arange(0, Z / 2, np.sqrt(3) * RADIUS): # DRAW IN VERTICAL COLUMNS
            ...
            for i in np.arange(-Z / 2, Z / 2, 3 / 2 * RADIUS):
                if counter % 2 == 0:
                    ...
                    R = np.sqrt(x0 ** 2 + y0 ** 2)
                    phi0 = np.arccos(x0 / R)
                    x1 = R * np.cos(phi0 + theta)
                    y1 = R * np.sin(phi0 + theta)
                    p1 = PolygonGeometry(
                        sides = 6,
                        radius = RADIUS,
                        x = x1,
    ...
    def screen_recorder(self, angle):
        ...
        pygame.image.save(screen_surf, f"../Visualizations_Frames/open_hand/{angle}.jpg")
    ...
    def update(self):
        if self.angle == 0:
            self.hex_tile2(phi = self.angle, theta = 0) 
