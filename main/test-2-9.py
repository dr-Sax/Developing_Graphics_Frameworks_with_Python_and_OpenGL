...
class Test(Base):
    ...
    def update(self):
        ...
        ### update data ###
        self.translation.data[0] = 0.75 * cos(self.time)  # change in x-pos
        self.translation.data[1] = 0.75 * sin(self.time)  # change in y-pos
        self.baseColor.data[0] = (sin(3 * self.time) + 1) / 2
        self.baseColor.data[1] = (sin(3 * self.time + 2.1) + 1) / 2
        self.baseColor.data[2] = (sin(3 * self.time + 4.2) + 2) / 2
        ### render scene ###
